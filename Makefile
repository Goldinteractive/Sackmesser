...
postcss:
	# autoprefix the css
	@ $(POSTCSS) --use autoprefixer --autoprefixer.browsers "> 5%" $(CSS_OUT)/style.scss.css -o $(CSS_OUT)/style.css

watch-postcss:
	@ $(POSTCSS) --use autoprefixer --autoprefixer.browsers "> 5%" $(CSS_OUT)/style.scss.css -o $(CSS_OUT)/style.css --watch
